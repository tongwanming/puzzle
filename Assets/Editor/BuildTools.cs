...
public class BuildTools : EditorWindow
{
    ...
    {
        get { return new Color[] {new Color(0, 0.75f, 1, 1), new Color(1, 1, 0.5f, 1)}; }
    }
    ...
    private void OnGUI()
    {
        ...
        strApplicationIdentifier = isSetting
            ? EditorGUILayout.TextField("包名", strApplicationIdentifier)
        ...
        if (isSetting)
        {
            if (GUILayout.Button("保存物料"))
            {
                ...
                PlayerSettings.bundleVersion =
                    PlayerSettings.bundleVersion == "0.1" ? "1.0.0" : PlayerSettings.bundleVersion;
            }
        }
    ...
    private void StartBuild()
    {
        ...
        string APKName = buildPath + strProductName + (isRelease ? "_release_" : "_") +
                         DateTime.Now.ToString("MMdd_HHmm") + "_"
    ...
    [MenuItem("Tools/安卓打包工具")]
    public static void showBuildPanel()
    {
        BuildTools bt = GetWindow<BuildTools>("安卓打包工具");
        bt.Show();
    }
}
...
namespace Hyperbyte
{
    ...
    public class GamePlayUI : Singleton<GamePlayUI>
    {
        ...
        IEnumerator TryRescueGameEnumerator(GameOverReason reason)
        {
            ...
            {
                AdManager.OnInterstitialClosedEvent += () =>
                {
                    if (CanRescueGame())
                    {
        ...
        /// 点击游戏道具
        /// </summary>
        public void OnClickGameProp()
        {
            gameProp.UseGameProp(gamePlay);
        }
        ...
        public void OnClickShowRewarded()
        {
            ...
            int adCount = PlayerPrefs.GetInt(dateStr);
            Debug.Log(dateStr + ":" + adCount);
            if (adCount > 3)
            {

                UIController.Instance.ShowTipAtPosition(Vector2.zero, new Vector2(0.5F, 0.5F), "Advertisements can be viewed up to four times a day", 3F);
                return;
            }
            if (AdManager.Instance.IsRewardedAvailable())
            {
                AdManager.OnRewardedAdRewardedEvent += OnRewardedAdRewarded;
                AdManager.Instance.ShowRewardedWithTag(clickVideoTag);
                PlayerPrefs.SetInt(dateStr, ++adCount);
            }
            else
            {
        ...
        {
            if (watchVidoTag == clickVideoTag)
            {
                Debug.Log("Ads ==> OnClickShowRewarded  OnRewardedAdRewarded  -- " + watchVidoTag);
